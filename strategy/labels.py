...
def compute_triple_barrier_labels(
    ...
    time_horizon_delta = pd.Timedelta(hours=horizon_delta)
    ...
    for i,event_date in tqdm(enumerate(event_index)):
    ...
    event_spans_labels = pd.DataFrame({'t1':event_spans,'label': label_series})
...
def compute_triple_barrier_labels1(
    ...
    time_horizon_delta = horizon_delta*60*60 # to timestamp seconds
...
def calc_triple_barrier(df0,
    ...
    event_spans_labels = compute_triple_barrier_labels1(price_series = df[col_series],
    ...
    # Concatenate labels to 
    df = pd.merge(df,event_spans_labels, right_index=True, left_index=True)
